...
Les paiements commencent après une certaine période de différé et continuent jusqu'au décès de l'assuré.

Formules de calcul des rentes viagères
---------------------------------------

Les calculs de rentes viagères impliquent l'utilisation de tables de mortalité pour estimer la probabilité de survie et déterminer la valeur actuelle des paiements futurs.

**Valeur actuelle d'une rente viagère**

La valeur actuelle d'une rente viagère (VA) est la somme des valeurs actuelles de tous les paiements futurs, pondérée par la probabilité de survie à chaque période.

.. math::

    VA = \sum_{t=0}^{n} \frac{PMT \cdot p_x}{(1 + r)^t}

où:

-   *VA* : Valeur actuelle de la rente viagère
-   *PMT* : Paiement périodique
-   *p_x* : Probabilité de survie à l'âge x
